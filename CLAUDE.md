...
* KNOWN ISSUE — vtiger-side, not a site bug: the enquiry form's `description` field is correctly sent from the frontend on every submission (confirmed by the founder via browser Network tab inspection of the raw multipart POST payload to `capture.php` — the field was present with the exact typed test value) but is not being saved on vtiger's end, despite vtiger's own admin panel showing the field mapping as correct. Root-caused down to: field name attribute correct and unaltered across every commit in this file's history, no client-side JS intercepts the native form POST, no CSS blocks the textarea — frontend is provably clean. Do not re-investigate this as a frontend bug in a future session; if it needs fixing, it requires vtiger-side support/configuration, not a code change here.
